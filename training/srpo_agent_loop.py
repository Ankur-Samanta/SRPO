...
class SRPOAgentLoop(ThoughtICSAgentLoop):
    ...
    async def run(self, sampling_params: dict, **kwargs) -> AgentLoopOutput:
        ...
        if my_slot == 0:
            ...
            try:
                ...
                # Opt-in branch-structure dump (no-op unless SCGRPO_BRANCH_DUMP_DIR set).
        ...
        # Also emit raw reward under scpo_reward so ics_metrics.py fresh/correction
        # accuracy tracking works without modification.
        output.extra_fields["scpo_reward"] = _meta["reward"] if _meta else 0.0
    ...
    async def _fill_rollout_buffer(
        ...
        ics_stats = {
            ...
            "fresh_chains": 0,
            "iter_oracle_correct": [],
            # Offline analysis: populated when SCGRPO_BRANCH_DUMP_DIR is set.
            "ics_loc_error_steps": [],
            "ics_loc_n_steps": [],
            "ics_loc_reasonings": [],
            "ics_loc_prompts": [],
