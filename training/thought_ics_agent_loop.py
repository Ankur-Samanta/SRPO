...
class ThoughtICSAgentLoop(ThoughtAgentLoop):
    ...
    async def run(self, sampling_params: dict, **kwargs) -> AgentLoopOutput:
        ...
        # This means SCGRPO eval is identical to TGRPO eval — both use thought_agent.
        ...
        if my_slot == 0:
            ...
            try:
                ...
                # No-op unless SCGRPO_BRANCH_DUMP_DIR is set. Silent on errors.
                dump_branch_group(
                    question=question,
        ...
        # Attach per-trajectory SCPO metadata (reward + suffix start for gradient masking)
        _scpo_meta = None
        if my_slot < len(buffer.buffer_meta):
            _scpo_meta = buffer.buffer_meta[my_slot]
        output.extra_fields["scpo_reward"] = _scpo_meta["reward"] if _scpo_meta else 0.0
        output.extra_fields["suffix_start_idx"] = _scpo_meta["suffix_start_idx"] if _scpo_meta else 0
    ...
    async def _fill_rollout_buffer(
        ...
        ics_stats = {
            ...
            # ics_error_steps. Only populated when SCGRPO_BRANCH_DUMP_DIR is set.
            "ics_loc_error_steps": [],
            "ics_loc_n_steps": [],
            "ics_loc_reasonings": [],
        ...
        _capture_loc = bool(os.environ.get("SCGRPO_BRANCH_DUMP_DIR", "").strip())
        ...
        while len(buffer) < n:
            ...
            if len(buffer) >= n:
                # Still need correctness for SCPO reward on the last fresh chain
