"""Opt-in branch structure logger for SRPO analysis.
...
Enable with env vars (all optional — no-op if SRPO_BRANCH_DUMP_DIR unset):
  SRPO_BRANCH_DUMP_DIR=/path/to/dir       # output directory (required to enable)
  SRPO_BRANCH_DUMP_EVERY=50               # dump every Nth prompt (default: 50)
  SRPO_BRANCH_DUMP_MAX=1000               # stop after N total dumps (default: unlimited)
...
def _resolve_output_path() -> Optional[Path]:
    ...
    dump_dir = os.environ.get("SRPO_BRANCH_DUMP_DIR", "").strip()
    if not dump_dir:
        return None
...
def dump_branch_group(
    ...
    try:
        every = int(os.environ.get("SRPO_BRANCH_DUMP_EVERY", "50"))
        max_dumps = int(os.environ.get("SRPO_BRANCH_DUMP_MAX", "0") or "0")
    except ValueError:
        every = 50
        max_dumps = 0
