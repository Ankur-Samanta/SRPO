"""Visualize SRPO rollout branching structure + per-thought credit signal.
...
root, while SRPO rollouts (ICS corrections share a prefix with their parent)
...
def render(record: dict, out_path: Path | None = None, figsize: tuple = (20, 12),
    ...
    title = (
        f"{mode_line}"
        f"SRPO rollout branches — {n_correct}/{len(rollouts)} correct\n"
