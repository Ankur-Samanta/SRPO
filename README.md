...
> Submit scripts default to the SLURM partition `q1`. Set `SLURM_PARTITION` to target your own
> cluster's partition, e.g. `SLURM_PARTITION=gpu bash batch_scripts/submit_srpo.sh olmo7b_numina_oly`.
...
bash batch_scripts/submit_srpo.sh olmo7b_numina_oly
# RRPO: random reset
bash batch_scripts/submit_srpo.sh olmo7b_numina_oly_rand
# code (LiveCodeBench v6 medium)
bash batch_scripts/submit_srpo.sh olmo7b_lcb_medium
```
