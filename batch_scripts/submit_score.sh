...
LAUNCH="${SCPO_DIR}/baselines/score/scripts/launch_training.sh"
...
submit() {
    ...
    sbatch --partition=q1 \
           ...
           --wrap="bash -c 'source ${CONDA_PATH}/etc/profile.d/conda.sh && conda activate ${CONDA_ENV} && cd ${SCPO_DIR} && ${env_prefix}${cmd}'"
...
run_or_submit() {
    ...
    if [ "$MODE" = "--local" ]; then
        ...
        submit "$job_name" "bash ${SCPO_DIR}/batch_scripts/submit_score.sh ${case_key} --local --training_seed=${TRAINING_SEED}" "$n_gpus" "$time_limit"
    fi
}
