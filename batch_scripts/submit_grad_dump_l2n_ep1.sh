...
#
# Usage:
#   ./batch_scripts/submit_grad_dump_l2n_ep1.sh             # SLURM
#   ./batch_scripts/submit_grad_dump_l2n_ep1.sh --local     # interactive
...
CONDA_ENV="srpo"
JOB_NAME="srpo_graddump_l2n_ep1"
EXP_NAME="grad_dump_l2n_ep1"
PROJECT="srpo_grad_dump"

ORIG_LORA="${SRPO_DIR}/checkpoints/srpo/lcb_medium_olmo7b_srpo_l2new_ep1/global_step_11/actor/lora_adapter"
LORA_INIT="${SRPO_DIR}/checkpoints/${PROJECT}/lora_init/lora_adapter"
TRAIN_CKPT_DIR="${SRPO_DIR}/checkpoints/${PROJECT}/${EXP_NAME}"
DUMP_DIR="${SRPO_DIR}/logs/srpo_per_token/${EXP_NAME}"
BRANCH_DIR="${SRPO_DIR}/logs/srpo_localizations/${EXP_NAME}"
...
export SRPO_BRANCH_DUMP_DIR="${BRANCH_DIR}"     # explicit (so launcher auto-derive logic is skipped)
export SRPO_BRANCH_DUMP_EVERY=1
export SRPO_BRANCH_DUMP_MAX=20
...
if [[ "$MODE" == "--local" ]]; then
    ...
    EXPORTS="${EXPORTS},SRPO_BRANCH_DUMP_DIR=${BRANCH_DIR},SRPO_BRANCH_DUMP_EVERY=1,SRPO_BRANCH_DUMP_MAX=20"
    ...
    sbatch --partition="${SLURM_PARTITION:-q1}" \
           ...
           --export="${EXPORTS}" \
           --wrap="bash -c 'source ${CONDA_PATH}/etc/profile.d/conda.sh && conda activate ${CONDA_ENV} && cd ${SRPO_DIR} && bash ${SRPO_DIR}/batch_scripts/submit_grad_dump_l2n_ep1.sh --local'"
