...
if [[ "$MODE" == "--local" ]]; then
    ...
    sbatch --partition=q1 \
           ...
           --export="${EXPORTS}" \
           --wrap="bash -c 'source ${CONDA_PATH}/etc/profile.d/conda.sh && conda activate ${CONDA_ENV} && cd ${SCPO_DIR} && bash ${SCPO_DIR}/batch_scripts/submit_grad_dump_l2n_ep1.sh --local'"
