...
OUT="${OUT:-${SCPO_DIR}/training/tests/loc_probe_${TAG}.jsonl}"
JOB_NAME="loc_probe_${TAG}"

CMD="python ${SCPO_DIR}/training/tests/probe_localization_prompts.py \
    --model ${MODEL} \
    --variants ${VARIANTS} \
    --out ${OUT}"
...
sbatch --partition=q1 \
       ...
       --wrap="bash -c 'source ${CONDA_PATH}/etc/profile.d/conda.sh && conda activate ${CONDA_ENV} && export TRANSFORMERS_OFFLINE=1 && cd ${SCPO_DIR} && ${CMD}'"

echo "Submitted: ${JOB_NAME}"
echo "Output: ${OUT}"
echo "Logs:   ${SCPO_DIR}/batch_scripts/logs/${JOB_NAME}.{out,err}"
