...
#
# Code (LiveCodeBench v6 medium):
#   olmo7b_lcb_medium       OLMo 7B      + SRPO      (2 GPUs)
#   olmo7b_lcb_medium_rand  OLMo 7B      + SRPO-rand (2 GPUs)
#
# Pass a training seed with: ./submit_srpo.sh <job> --training_seed N

set -e

SRPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LAUNCH="${SRPO_DIR}/training/scripts/launch_srpo_training.sh"
...
SHARED_OVERRIDES=(
    ...
    # 2 epochs (consistent with SRPO/SRPO runs)
...
export SRPO_BRANCH_DUMP_EVERY=1
...
# olmo7b LCB-medium seed variants: srpo l2new and srpo-rand
...
run_or_submit() {
    ...
    if [ "$MODE" = "--local" ]; then
        ...
        submit "$job_name" "bash ${SRPO_DIR}/batch_scripts/submit_srpo.sh ${case_key} --local --training_seed=${TRAINING_SEED}${ICS_LOC_TEMP_VAL:+ --loc_temp=${ICS_LOC_TEMP_VAL}}" "$n_gpus" "$time_limit"
    fi
}
...
case $JOB in
    ...
    # LiveCodeBench Medium: SRPO (canonical) — clean aliases for the standard self-localized run
    olmo7b_lcb_medium)         run_or_submit "srpo_olmo7b_lcbm"  cmd_olmo7b_lcb_medium_l2new  olmo7b_lcb_medium   2 48:00:00 ;;
    qwen14b_lcb_medium)        run_or_submit "srpo_qwen14b_lcbm" cmd_qwen14b_lcb_medium_l2new qwen14b_lcb_medium  4 48:00:00 ;;
    ...
    list)
        echo "Primary jobs (paper configs; default seed 42):"
        echo "  Math (NuminaMath Olympiads):"
        echo "    SRPO:      olmo7b_numina_oly       qwen14b_numina_oly"
        echo "    RRPO:      olmo7b_numina_oly_rand  qwen14b_numina_oly_rand"
        echo "    Batch:     all_oly  (7B models only)"
        echo "  Code (LiveCodeBench v6 medium):"
        echo "    SRPO:      olmo7b_lcb_medium       qwen14b_lcb_medium"
        echo "    RRPO:      olmo7b_lcb_medium_rand"
        echo ""
        echo "Ablations: append _nm (no-mask), _srpo2x4 / _srpo_1x8 (sampling split),"
        echo "           _srpo_clip[_sm|_kl] (clipped surrogate) to numina_oly jobs."
        echo ""
        echo "Set a training seed with: $0 <job> --training_seed N"
        exit 0
        ;;
    *)
