...
LAUNCH_TREE="${SCPO_DIR}/baselines/spo/scripts/launch_tree_training.sh"
...
run_or_submit() {
    ...
    if [ "$MODE" = "--local" ]; then
        ...
        submit "$job_name" "bash ${SCPO_DIR}/batch_scripts/submit_spo.sh ${case_key} --local --training_seed=${TRAINING_SEED}" "$n_gpus" "$time_limit"
    fi
}
