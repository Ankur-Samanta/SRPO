...
if [[ "$MODE" == "--local" ]]; then
    ...
    sbatch --partition="${SLURM_PARTITION:-q1}" \
           ...
           --export="${EXPORTS}" \
           --wrap="bash -c 'source ${CONDA_PATH}/etc/profile.d/conda.sh && conda activate ${CONDA_ENV} && cd ${SRPO_DIR} && bash ${SRPO_DIR}/batch_scripts/submit_grad_dump_l2n_ep1_v2.sh --local'"
