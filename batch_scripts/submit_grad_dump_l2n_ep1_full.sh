...
export SRPO_BRANCH_DUMP_DIR="${BRANCH_DIR}"
export SRPO_BRANCH_DUMP_EVERY=1
export SRPO_BRANCH_DUMP_MAX=5000        # ~32 prompts × ~11 steps ≈ 352; 5k = generous headroom
...
if [[ "$MODE" == "--local" ]]; then
    ...
    EXPORTS="${EXPORTS},SRPO_BRANCH_DUMP_DIR=${BRANCH_DIR},SRPO_BRANCH_DUMP_EVERY=1,SRPO_BRANCH_DUMP_MAX=5000"
    ...
    sbatch --partition="${SLURM_PARTITION:-q1}" \
           ...
           --export="${EXPORTS}" \
           --wrap="bash -c 'source ${CONDA_PATH}/etc/profile.d/conda.sh && conda activate ${CONDA_ENV} && cd ${SRPO_DIR} && bash ${SRPO_DIR}/batch_scripts/submit_grad_dump_l2n_ep1_full.sh --local'"
