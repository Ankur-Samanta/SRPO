...
LAUNCH="${SRPO_DIR}/baselines/critique_grpo/scripts/launch_training.sh"
...
run_or_submit() {
    ...
    if [ "$MODE" = "--local" ]; then
        ...
        submit "$job_name" "bash ${SRPO_DIR}/batch_scripts/submit_critique_grpo.sh ${case_key} --local --training_seed=${TRAINING_SEED}" "$n_gpus" "$time_limit"
    fi
}
